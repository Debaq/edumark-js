...
function extractFrontmatter(source) {
  ...
  let frontmatter = {};
  try {
    const parsed = import_js_yaml.default.load(yamlStr);
...
function createParser() {
  const md = new import_markdown_it.default({ html: false, linkify: false });
...
var import_markdown_it3 = __toESM(require("markdown-it"), 1);

// src/renderer/block-renderers.ts
var import_markdown_it2 = __toESM(require("markdown-it"), 1);
var inlineMd = new import_markdown_it2.default({ html: false });
...
`;
  ...
  diagram(block) {
    const parts = [];
    const zoom = block.attributes.zoom;
    const zoomAttr = zoom ? ` data-edm-zoom="${esc2(zoom)}"` : "";
    if (block.diagramCode) {
      if (block.diagramCode.language === "mermaid") {
        parts.push(`<div class="edm-diagram-render"${zoomAttr}><pre class="mermaid">${esc2(block.diagramCode.code)}</pre></div>`);
        ...
        parts.push(`<div class="edm-diagram-render"${zoomAttr}><pre class="edm-diagram-code" data-language="${esc2(block.diagramCode.language)}">${esc2(block.diagramCode.code)}</pre></div>`);
      }
    }
  ...
  <div class="edm-embed-frame" data-embed-type="${esc2(type)}">
...
function render(doc, _options = {}) {
  ...
  const md = new import_markdown_it3.default({ html: false });
  md.use(inlineRefPlugin);
  md.use(mathPlugin);
  md.use(htmlCommentPlugin);
  md.enable("table");
