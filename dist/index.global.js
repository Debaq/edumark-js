  ...
  function storeMappingPair(state, _result, overridableKeys, keyTag, keyNode, valueNode, startLine, startLineStart, startPos) {
    ...
    if (Array.isArray(keyNode)) {
      ...
      for (index = 0, quantity = keyNode.length; index < quantity; index += 1) {
        ...
        if (typeof keyNode === "object" && _class3(keyNode[index]) === "[object Object]") {
...
`;
  ...
  function inlineRefPlugin(md) {
    ...
    md.renderer.rules["edm_ref"] = (tokens, idx) => {
      ...
      const text2 = token.meta.text;
      const href = file ? `${file}#${id2}` : `#${id2}`;
      const label = text2 || id2;
  ...
  function processChildren(children, TokenClass) {
    ...
    for (const token of children) {
      ...
      let lastIndex = 0;
      let match2;
      while ((match2 = REF_RE.exec(token.content)) !== null) {
        if (match2.index > lastIndex) {
          const textToken = new TokenClass("text", "", 0);
          textToken.content = token.content.slice(lastIndex, match2.index);
          result.push(textToken);
        }
        const inner = match2[1].trim();
        const { id: id2, file, text: text2 } = parseRefInner(inner);
        const refToken = new TokenClass("edm_ref", "", 0);
        refToken.meta = { id: id2, file, text: text2 };
  ...
  function parseRefInner(inner) {
    ...
    if (spaceIdx === -1) {
      ...
      text2 = inner.slice(spaceIdx + 1).trim();
      const hashIdx = ref.indexOf("#");
      if (hashIdx !== -1) {
        file = ref.slice(0, hashIdx);
        id2 = ref.slice(hashIdx + 1);
  ...
  function processMathInline(children, TokenClass) {
    ...
    for (const token of children) {
      ...
      let lastIndex = 0;
      let match2;
      while ((match2 = MATH_INLINE_RE.exec(token.content)) !== null) {
        if (match2.index > lastIndex) {
          const t = new TokenClass("text", "", 0);
          t.content = token.content.slice(lastIndex, match2.index);
          result.push(t);
        }
        const mathToken = new TokenClass("edm_math_inline", "", 0);
        mathToken.content = match2[1].trim();
  ...
  function parseDefinitions(content) {
    ...
    for (const line of lines) {
      ...
      if (match2) {
        results.push({ term: match2[1].trim(), definition: match2[2].trim() });
      }
    }
    return results;
  }
  ...
  function parseHeroFields(content) {
    ...
    for (const line of lines) {
      ...
      const match2 = trimmed.match(/^(\w[\w-]*):\s*(?:"([^"]*)"|(.+))$/);
      if (match2) {
        const key = match2[1];
        const value = match2[2] !== void 0 ? match2[2] : match2[3].trim();
        fields[key] = value;
      }
    }
  ...
  function parseImageFields(content) {
    ...
    for (const line of lines) {
      const trimmed = line.trim();
      if (!trimmed) continue;
      const match2 = trimmed.match(/^(\w+):\s*(?:"([^"]*)"|(.+))$/);
      if (match2) {
        const key = match2[1];
        const value = match2[2] !== void 0 ? match2[2] : match2[3].trim();
        fields[key] = value;
  ...
  function parseDiagram(content) {
    ...
    const code2 = fenceMatch[2].trim();
    const description = content.slice(0, fenceMatch.index).trim();
    return {
      description: description || "",
      diagramCode: { language, code: code2 }
    };
  }

  // src/blocks/question.ts
  function parseQuestion(content, type2) {
    const lines = content.split("\n");
    const stemLines = [];
    const options = [];
    const answers = [];
    ...
    for (const line of lines) {
      ...
      if (trimmed.startsWith("=") || trimmed.startsWith("~")) {
        ...
        let feedback;
        if (feedbackIdx !== -1) {
          text2 = rest.slice(0, feedbackIdx).trim();
          feedback = rest.slice(feedbackIdx + 1).trim();
        } else {
          text2 = rest;
        }
        options.push({ correct, text: text2, feedback });
      } else {
        if (!inOptions) {
  ...
  function createParser() {
    const md = new lib_default({ html: false, linkify: false });
  ...
  function parse(source, options = {}) {
    ...
    if (options.resolveInclude) {
      text2 = resolveIncludes(text2, options.resolveInclude);
    }
    const { frontmatter, content } = extractFrontmatter(text2);
  ...
  function buildBlocks(tokens, source, mode) {
    ...
    for (let i = 0; i < tokens.length; i++) {
      ...
      if (token.type === "edm_container_open") {
        ...
        block2.content = rawContent;
        parseBlockInternals(block2);
        stack[stack.length - 1].push(block2);
        stack.push(block2.children);
        currentBlock = block2;
  ...
  function parseBlockInternals(block2) {
    switch (block2.blockType) {
      case "hero": {
        const heroResult = parseHeroFields(block2.content);
        block2.fields = heroResult.fields;
        block2.topics = heroResult.topics;
        break;
      }
      case "definition":
        block2.definitions = parseDefinitions(block2.content);
        break;
      case "image":
        block2.fields = parseImageFields(block2.content);
        break;
      case "diagram": {
        const result = parseDiagram(block2.content);
        block2.description = result.description;
      ...
      case "question": {
        ...
        block2.content = result.stem;
        block2.options = result.options;
        block2.answers = result.answers;
        break;
      }
    }
  }
  function buildRefMap(blocks, tokens) {
    ...
    function getNumber(type2) {
      const n = (counters.get(type2) || 0) + 1;
      counters.set(type2, n);
      return n;
    }
    function walkBlocks(blockList) {
      for (const block2 of blockList) {
        if (block2.attributes.id) {
          const num = getNumber(block2.blockType);
          refs.set(block2.attributes.id, {
            type: "block",
            blockType: block2.blockType,
            title: block2.attributes.title || void 0,
            number: num
          });
        }
        walkBlocks(block2.children);
      }
    }
    walkBlocks(blocks);
  ...
  var inlineMd = new lib_default({ html: false });
  inlineMd.use(mathPlugin);
  inlineMd.enable("table");
  function renderInline(text2) {
    return inlineMd.renderInline(text2);
  }
  function renderMarkdown(text2) {
    return inlineMd.render(text2);
  }
  function id(block2) {
    return block2.attributes.id ? ` id="${esc2(block2.attributes.id)}"` : "";
  }
  function esc2(s) {
    return s.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;");
  }
  ...
  function blockCard(block2, inner, extraClass) {
    const type2 = block2.blockType;
    const icon = ICONS[type2] || "";
    const label = LABELS[type2] || type2;
    const title = block2.attributes.title;
    const cls = extraClass ? ` ${extraClass}` : "";
    return `<article class="edm-card edm-${type2}${cls}"${id(block2)} role="region" aria-label="${esc2(label)}">
...
`;
  ...
  function renderBlock(block2, childrenHtml) {
    const renderer = RENDERERS[block2.blockType];
    if (renderer) return renderer(block2, childrenHtml);
    return renderGeneric(block2, childrenHtml);
  }
  function renderGeneric(block2, childrenHtml) {
    return blockCard(block2, `${renderMarkdown(block2.content)}${childrenHtml}`);
  }
  var RENDERERS = {
    hero(block2) {
      const f = block2.fields || {};
      const topics = block2.topics || [];
      const parts = [];
      parts.push(`<header class="edm-hero"${id(block2)}>`);
      const badges = [];
    ...
    objective(block2, childrenHtml) {
      return blockCard(block2, `${renderMarkdown(block2.content)}${childrenHtml}`);
    },
    definition(block2, childrenHtml) {
      const defs = block2.definitions || [];
      if (defs.length === 0) return renderGeneric(block2, "");
  ...
  <dd>${renderInline(d.definition)}</dd>
      ...
      return blockCard(block2, `<dl class="edm-def-list">
${items}
</dl>${childrenHtml}`);
    },
    "key-concept"(block2, childrenHtml) {
      return blockCard(block2, `${renderMarkdown(block2.content)}${childrenHtml}`);
    },
    warning(block2, childrenHtml) {
      return blockCard(block2, `${renderMarkdown(block2.content)}${childrenHtml}`);
    },
    note(block2, childrenHtml) {
      return blockCard(block2, `${renderMarkdown(block2.content)}${childrenHtml}`);
    },
    example(block2, childrenHtml) {
      return blockCard(block2, `${renderMarkdown(block2.content)}${childrenHtml}`);
    },
    exercise(block2, childrenHtml) {
      return blockCard(block2, `${renderMarkdown(block2.content)}${childrenHtml}`);
    },
    solution(block2) {
...
<div class="edm-card-body">
${renderMarkdown(block2.content)}
</div>
</details>
`;
    },
    application(block2, childrenHtml) {
      return blockCard(block2, `${renderMarkdown(block2.content)}${childrenHtml}`);
    },
    comparison(block2, childrenHtml) {
      return blockCard(block2, `<div class="edm-table-wrap">${renderMarkdown(block2.content)}</div>${childrenHtml}`);
    },
    image(block2) {
      const f = block2.fields || {};
      const src = f.file || "";
  ...
  ${captionInner ? `<figcaption>${captionInner}</figcaption>` : ""}
    ...
    question(block2) {
      ...
      parts.push(`<div class="edm-q-type-badge">${typeLabels[type2] || type2}</div>`);
      if (block2.content) {
        parts.push(`<div class="edm-q-stem">${renderMarkdown(block2.content)}</div>`);
      }
      if (type2 === "choice") {
        const options = block2.options || [];
        const multipleCorrect = options.filter((o) => o.correct).length > 1;
        const inputType = multipleCorrect ? "checkbox" : "radio";
        const name = block2.attributes.id || "q";
  ...
  <span class="edm-q-text">${renderInline(opt.text)}</span>
        ...
        parts.push("</div>");
      } else if (type2 === "true-false") {
        const answer = block2.answers && block2.answers[0] || "";
        const name = block2.attributes.id || "q";
        const opts = block2.options || [];
        parts.push('<div class="edm-q-options">');
  ...
  <span class="edm-q-text">Falso</span>
      ...
      } else if (type2 === "open" || type2 === "case") {
        if (block2.answers && block2.answers.length > 0) {
          parts.push(`<details class="edm-q-answer">
<summary>Ver respuesta modelo</summary>
<div class="edm-q-answer-body">
${block2.answers.map((a) => renderMarkdown(a)).join("\n")}
</div>
</details>`);
        }
      }
    ...
    mnemonic(block2, childrenHtml) {
      return blockCard(block2, `${renderMarkdown(block2.content)}${childrenHtml}`);
    },
    history(block2) {
      let meta = "";
      if (block2.attributes.characters || block2.attributes.year) {
        const metaParts = [];
        if (block2.attributes.characters) metaParts.push(`<span class="edm-hist-who"><span aria-hidden="true">\u{1F464}</span> ${esc2(block2.attributes.characters)}</span>`);
        if (block2.attributes.year) metaParts.push(`<span class="edm-hist-when"><span aria-hidden="true">\u{1F4C5}</span> ${esc2(block2.attributes.year)}</span>`);
        meta = `<div class="edm-hist-meta">${metaParts.join("")}</div>
`;
      }
      return blockCard(block2, `${meta}${renderMarkdown(block2.content)}`);
    },
    summary(block2, childrenHtml) {
      return blockCard(block2, `${renderMarkdown(block2.content)}${childrenHtml}`);
    },
    reference(block2, childrenHtml) {
      return blockCard(block2, `${renderMarkdown(block2.content)}${childrenHtml}`);
    },
    aside(block2, childrenHtml) {
      return blockCard(block2, `${renderMarkdown(block2.content)}${childrenHtml}`);
    },
    "teacher-only"(block2, childrenHtml) {
      return blockCard(block2, `${renderMarkdown(block2.content)}${childrenHtml}`);
    },
    "student-only"(block2, childrenHtml) {
      return blockCard(block2, `${renderMarkdown(block2.content)}${childrenHtml}`);
    },
    math(block2) {
      return blockCard(block2, renderMathBlock(block2.content));
    },
    include(block2) {
      const file = block2.attributes.file || "";
      const title = block2.attributes.title || "";
      const anchorId = file.replace(/\.edm$/, "").replace(/[^a-zA-Z0-9_-]/g, "-");
      const label = title || block2.content.trim() || file.replace(/\.edm$/, "").replace(/[_-]/g, " ");
      return `<div class="edm-include-link"${id(block2)}>
...
`;
  ...
  function buildNumbering(blocks) {
    ...
    function walk(blockList) {
      for (const block2 of blockList) {
        if (NUMBERED_TYPES.has(block2.blockType) && block2.attributes.id) {
          const n = (counters.get(block2.blockType) || 0) + 1;
          counters.set(block2.blockType, n);
          const prefix = TYPE_LABELS[block2.blockType] || block2.blockType;
          labels.set(block2.attributes.id, `${prefix} ${n}`);
        }
        walk(block2.children);
      }
    }
    walk(blocks);
    return labels;
  ...
  function render(doc, _options = {}) {
    ...
    const md = new lib_default({ html: false });
    md.use(inlineRefPlugin);
    md.use(mathPlugin);
    md.use(htmlCommentPlugin);
    md.enable("table");
    ...
    while (i < tokens.length) {
      if (tokens[i].type === "edm_container_open") {
        ...
        const block2 = blocksByKey.get(key);
        if (block2) {
          const childrenHtml = renderChildrenHtml(block2);
          parts.push(renderBlock(block2, childrenHtml));
        }
        let depth = 1;
        i++;
        while (i < tokens.length && depth > 0) {
  ...
  function renderChildrenHtml(block2) {
    return block2.children.map(
  ...
  function buildBlockIndex(blocks) {
    ...
    function walk(list2) {
      for (const block2 of list2) {
        const key = blockKey(block2.blockType, block2.attributes);
        map3.set(key, block2);
        walk(block2.children);
      }
    }
